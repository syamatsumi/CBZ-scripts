...
set PS1WEBP=CBZts_Webpconv.ps1
set WEBPQUALITY=90
...
for %%C in (cb7 cbr cbt cbz) do (
  ...
  for %%A in (*.%%C) do (
    ...
    rem ## WORKPATH直下のアイテムが1つだけで、それがフォルダなら1階層フラット化.
    set "ONLYITEM="
    set "ITEMCNT=0"
    for /f "delims=" %%I in ('dir /b /a "!WORKPATH!" 2^>nul') do set "ONLYITEM=%%I"
    for /f %%C in ('dir /b /a "!WORKPATH!" 2^>nul ^| find /c /v ""') do set "ITEMCNT=%%C"
    if "!ITEMCNT!"=="1" if exist "!WORKPATH!\!ONLYITEM!\" (
      echo [FLATTEN] !WORKPATH!\!ONLYITEM!
      robocopy "!WORKPATH!\!ONLYITEM!" "!WORKPATH!" /E /MOVE /NFL /NDL /NJH /NJS
      rmdir "!WORKPATH!\!ONLYITEM!" 2>nul
    )

    rem ## 展開した画像を WebP に変換し、可能な範囲でメタデータと日時を引き継ぐ.
    if not exist "%WEBPPATH%" (
      echo [ERROR] %PS1WEBP% が見つかりません.
      pause
      exit /b 1
    )
    pwsh -NoProfile -ExecutionPolicy Bypass -File "%WEBPPATH%" -TgtRoot "!WORKPATH!" -Quality %WEBPQUALITY%
    if errorlevel 1 (
      echo [ERROR] WebP 変換に失敗しました: !ORGARCNAME!
      echo 元アーカイブは変更していません. 作業フォルダを残して終了します.
      pause
      exit /b 1
    )

...
$scriptDir = if ($TgtRoot -eq ".") { Split-Path -Parent $MyInvocation.MyCommand.Definition } else { $TgtRoot }

# 特定のファイルを事前に削除（掃除）
$wildcardPath = Join-Path $scriptDir '*'
Get-ChildItem -LiteralPath $scriptDir -Recurse -File -Force -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -in @("desktop.ini","Thumbs.db",".DS_Store") } |
  ForEach-Object {
    ...
    ForEach-Object {
      if (-not (Get-ChildItem -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue | Select-Object -First 1)) {
        try {
          ...
          $deleted++
          Write-Host "削除: $($_.FullName)" -NoNewline -ForegroundColor Yellow
        } catch {
          Write-Warning "削除失敗: $($_.FullName) -> $($_.Exception.Message)" -ForegroundColor Red
        }
      }
    }
} while ($deleted -gt 0)

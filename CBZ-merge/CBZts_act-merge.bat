...
set EXTDIR=_extmerge
set TEMPDIR=_tmp%RANDOM%
set PS1DEDUPE=CBZts_dedup.ps1
...
set DEDUPEPATH=%~dp0%PS1DEDUPE%
set RESYPATH=%~dp0%PS1RESY%
set WIPEPATH=%~dp0%PS1WIPE%

rem ### 重要な外部スクリプトのチェック
if not exist "%DEDUPEPATH%" (
    echo [ERROR] "%DEDUPEPATH%" が見つかりません。このまま終了します.
    pause
    exit /b 1
)
rem ### 事前対応処理 ###
...
rem ### 分解結合開始.
echo デリミネータ「%SPLIT_KEY%」より左側だけの名前（なければ全体）で解凍＆結合.

rem ### 展開対象の事前カウント.
set TOTAL=0
set COUNT=0
for %%C in (7z arj cab cb7 cbr cbt cbz lzh rar tar uue zip) do (
...
for %%C in (7z arj cab cb7 cbr cbt cbz lzh rar tar uue zip) do (
  ...
  for %%A in (*.%%C) do (
    ...
    set ORGARCNAME=%%~nA
    set ORGARCPATH=%%~fA
    rem 文字列の置換でデリミネーターを｜に変換して!TMPNAME!に格納.
    set TMPNAME=!ORGARCNAME:%SPLIT_KEY%=^|!
    rem 先頭トークンを取得して!BASENAME!に格納.
    for /f "tokens=1 delims=|" %%B in ("!TMPNAME!") do set BASENAME=%%B
    rem %SPLIT_KEY%が!ORGARCNAME!にない場合は!BASENAME!が.
    rem 未定義になるため!BASENAME!に!ORGARCNAME!を入れる.
    if not defined BASENAME set BASENAME=!ORGARCNAME!
    rem !WORKPATH!確定、フォルダの作成.
    set WORKPATH=%TPDPATH%\!BASENAME!
...
echo.
echo ++++++++++++++++++++++++++++++++++++++++++++++++++
echo TRY MERGE FILES 
echo   Files with the same name in the merged folder
echo   have been saved under different names.
echo   However, if they have the same content,
echo   they will be deleted during this process.
echo ++++++++++++++++++++++++++++++++++++++++++++++++++
echo.
pwsh -NoProfile -ExecutionPolicy Bypass -File "%DEDUPEPATH%" -TgtRoot "%TPDPATH%"
...
for /d %%D in (*) do (
  ...
  if not exist "%EXDPATH%\!ORGARCNAME!.cbz" (
    ...
    if %errorlevel%==0 (
      ...
      pwsh -NoLogo -NoProfile -Command ^
        "$scfltime = (Get-Item -LiteralPath \"!WORKPATH!\").CreationTime; " ^

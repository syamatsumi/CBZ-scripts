...
for %%F in (*.zip) do (
  ...
  pushd "%TEMPDIR%"
  pwsh -NoProfile -ExecutionPolicy Bypass -File "%ps1file%" -NoConfirm
  del /Q "%ps1file%"
  popd
  ...
  if !ITEMCOUNT!==1 (
    for /f "delims=" %%X in ('dir /b /a "%TEMPDIR%"') do (
      set DEST=%~dp0%EXTDIR%\%%X
      if not exist "!DEST!" (
        move "%TEMPDIR%\%%X" "%EXTDIR%\"
      ) else (
        set DEST=%~dp0%EXTDIR%\%%X_!TS!
        move "%TEMPDIR%\%%X" "!DEST!"
      )
      ...
      if exist "!DEST!\NUL" (
        ...
        pwsh -NoProfile -Command ^
        "(Get-Item -LiteralPath '!DEST!').CreationTime=(Get-Item -LiteralPath '%%~fF').LastWriteTime"
      )
      rmdir /s /q "%TEMPDIR%"
    )
  ) else (
    rem 複数項目：移動後の最終フォルダに対して適用
    set DEST=%~dp0%EXTDIR%\%%~nF
    if not exist "!DEST!" (
      move "%TEMPDIR%" "!DEST!"
    ) else (
      set DEST=%~dp0%EXTDIR%\%%~nF_!TS!
      move "%TEMPDIR%" "!DEST!"
    )
    pwsh -NoProfile -Command ^
    "(Get-Item -LiteralPath '!DEST!').LastWriteTime=(Get-Item -LiteralPath '%%~fF').LastWriteTime"

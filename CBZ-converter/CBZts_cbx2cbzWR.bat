...
for %%C in (cb7 cbr cbt cbz) do (
  ...
  for %%A in (*.%%C) do (
    ...
    echo ######################################################################
    echo.
    rem overwrite-o+ skip-o- rename(to)-or warn-ow 
    "%WINRAR%" x -or -tk -ibck -p%ZIPPASS% "!ORGARCPATH!" "!WORKPATH!\"
    ...
    echo ######################################################################
    echo.
    "%WINRAR%" a -afzip -ep1 -t -tk -r -df -ibck "!OUTPUTARCPATH!.zip" "!WORKPATH!\*"
    if %errorlevel%==0 (
